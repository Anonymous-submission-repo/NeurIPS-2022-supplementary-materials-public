...
class BlendSearch(Searcher):
    ...
    def __init__(
        ...
        use_lexicographic_preference=False,
    ):
        """Constructor.
    ...
    def _init_search(self):
        ...
        if self.use_lexicographic_preference is False:
    ...
    def _create_thread(self, config, result, space):
        if isinstance(self._ls.metric, list):
            obj = {}
            for k in self._ls.metric:
                obj[k] = result[k]
        else:
    ...
    def suggest(self, trial_id: str) -> Optional[Dict]:
        ...
        if self.use_lexicographic_preference is True:
            if self._trial_proposed_by[trial_id] > 0:
                config["incumbent_comparision"] = self._search_thread_pool[
                    self._trial_proposed_by[trial_id]
                ]._search_alg.best_obj
                config["Lbest"] = self._search_thread_pool[
                    self._trial_proposed_by[trial_id]
                ]._search_alg.Lbest
            else:
                config["incumbent_comparision"] = None
                config["Lbest"] = None
        return config
...
class BlendSearchTuner(BlendSearch, NNITuner):
    ...
    def update_search_space(self, search_space):
        ...
        self._ls = self.LocalSearch(
            ...
            seed=self._ls.seed,
            use_lexicographic_preference=self.use_lexicographic_preference,
        )
        if self._gs is not None:
            self._gs = GlobalSearch(
                space=config,
