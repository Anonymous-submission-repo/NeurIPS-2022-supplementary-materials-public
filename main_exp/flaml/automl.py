...
class AutoMLState:
    ...
    def _compute_with_config_base(config_w_resource, state, estimator): 
            ...
            if metric_order == -1: # final
                ...
                if bound > c: 
                    ...
                    if result <= c:
                        ...
                        return [-1,metric_order]                      
        if state.use_lexicographic_preference is True:
            incumbent_comparision = config_w_resource["incumbent_comparision"]
            del config_w_resource["incumbent_comparision"]
            Lbest = config_w_resource["Lbest"]
            del config_w_resource["Lbest"]
            if state.mode != None:
                state.metric_op = {}
                for k, v in state.mode.items():
                    state.metric_op[k] = -1.0 if v == "max" else 1.0
        if not state.only_result:
            order = {}
            index_order = 0
            for key in state.preference_list:
                order[key] = index_order
                index_order  = index_order+1
            last_key = state.preference_list[-1]
            order[last_key] = -1
        ...
        result = {
            ...
            "val_loss": val_loss,
            "train_time":np.log2(train_time).item(),
            "trained_estimator": trained_estimator,
        }
        if sampled_weight is not None:
            state.fit_kwargs["sample_weight"] = weight
        if state.only_result:
            return result
        if state.use_lexicographic_preference is True:
            # update Lbest 
            if Lbest == None:
                ...
                for key,value in result.items():
                    if key in state.preference_list:
            ...
            if incumbent_comparision is None:
                ...
                for metric in state.preference_list:
                    op = state.metric_op[metric]
                    c = state.C[metric]
                    t = state.T[metric]
                    ...
                    if comp_result[0] == 0:
                        ...
                        break
                if comp_result[0] == 0:
                    metric = state.preference_list[0]
                    op = state.metric_op[metric]
                    bound = incumbent_comparision[metric]
                    if result[metric]*op < bound:
                        result["lexicographic_preference"] = True
                        comp_result[1] = -2
                    else:
                        result["lexicographic_preference"] = False
                if result["lexicographic_preference"]:
                    if comp_result[1] == -1:
                        start_index = len(state.preference_list)-1
                    elif comp_result[1] == -2:
                        start_index = len(state.preference_list)
                        result["Lbest"] = Lbest
                    else:
                        start_index = comp_result[1]
                    for metric_index in range(start_index,len(state.preference_list)):
                        metric_name = state.preference_list[metric_index]
...
class AutoML(BaseEstimator):
    ...
    def __init__(self, **settings):
        ...
        self._state.use_lexicographic_preference = False
        self._state.only_result = False
