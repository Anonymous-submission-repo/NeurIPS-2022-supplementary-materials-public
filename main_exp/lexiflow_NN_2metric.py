    ...
    if incumbent_comparision is None:
        ...
        if result["lexicographic_preference"]:
            if comp_result[1] == -1:
                ...
                start_index = len(preference_list)
                result["Lbest"] = Lbest
            else:
...
def test(seed, c_value, toler_value,preference_list):
    ...
    c_str = "".join(c_list)
    path = "./results/mnist_incu_bigTime/seed-{seed}_algorithm-CFO_C-{c_str}_toler-{toler_str}/".format(seed = seed,c_str=c_str,toler_str = toler_str)
...
preference_list = ["error_rate","flops"]
