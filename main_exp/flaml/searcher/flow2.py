...
class FLOW2(Searcher):
    ...
    def __init__(
        ...
        mode: Optional[Union[str, dict]] = None,
        ...
        use_lexicographic_preference: Optional[bool] = False,
    ):
        """Constructor.
        ...
        """
        if use_lexicographic_preference is False:
            if mode:
        ...
        if isinstance(mode, str):
            ...
            for k, v in mode.items():
                self.metric_op[k] = -1.0 if v == "max" else 1.0
        self.space = space or {}
        ...
        self.use_lexicographic_preference = use_lexicographic_preference
    ...
    def create(
        ...
        flow2 = self.__class__(
            ...
            self.cost_attr,
            self.seed + 1,
            self.use_lexicographic_preference,
        )
        if self.use_lexicographic_preference is True:
            flow2.best_obj = {}
            for k, v in obj.items():
                flow2.best_obj[k] = v * self.metric_op[k]
        else:
    ...
    def set_search_properties(
        ...
        if mode and isinstance(mode, str):
            ...
            if mode == "max":
                ...
                self.metric_op = 1.0
        elif mode and isinstance(mode, dict):
            self.metric_op = {}
            for k, v in mode.items():
                self.metric_op[k] = -1.0 if v == "max" else 1.0
        if config:
    ...
    def on_trial_complete(
        ...
        if not error and result:
            update_flag = False
            if self.use_lexicographic_preference is True:
                bool_flag = result.get("lexicographic_preference")
                # incumbent_test = result.get("incumbent_comparision")
                self.Lbest = result.get("Lbest")
                obj = {}
                for k in self._metric:
                    obj[k] = result[k]
                if not all(j is None for i, j in obj.items()):
                    for idx, val in obj.items():
                        obj[idx] = self.metric_op[idx] * val
                update_flag = obj is not None and (bool_flag or self.best_obj is None)
            else:
                obj = result.get(self._metric)
                if obj:
                    obj *= self.metric_op
                update_flag = obj is not None and (
                    obj < self.best_obj or self.best_obj is None
                )
            if update_flag:
                self.best_obj = obj
        ...
        if proposed_by == self.incumbent:
            # print("k",self._num_complete4incumbent)
            # print("dim",self.dir)
